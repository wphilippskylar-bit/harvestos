...
import {
  demoOrg, demoCrops, demoBatches, demoPurchases, demoChannels, demoSales, demoGoals,
} from "@/lib/demo-data";
...
export async function getOrgContext(): Promise<OrgContext> {
  ...
  const { data: membership } = await supabase
    .from("memberships")
    .select("org_id, role, organizations(name, plan_tier, seat_limit, batch_id_prefix)")
    .eq("user_id", user.id)
  ...
  const org = membership.organizations as unknown as { name: string; plan_tier: string; seat_limit: number; batch_id_prefix: string } | null;
  return {
